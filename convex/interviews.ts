...
export const getMyInterviewsWithInterviewers = query({
  handler: async (ctx) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) return [];
    ...
    // Get all users to find interviewer information
    const allUsers = await ctx.db.query("users").collect();

    // Enrich interviews with interviewer information
    const interviewsWithInterviewers = interviews.map((interview) => {
      const interviewers = interview.interviewerIds.map((interviewerId) => {
        const interviewer = allUsers.find((user) => user.clerkId === interviewerId);
        return {
    ...
    return interviewsWithInterviewers;
  },
});